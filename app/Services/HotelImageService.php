...
{
    private const DIRECTORY = 'travel_media/hotels';
    ...
    public function store(UploadedFile $file, string $hotelName): array
    {
        ...
        try {
            ...
            if (! rename($tempPath, $originalPath)) {
                throw new RuntimeException('No se pudo preparar la imagen del hotel.');
            }
            $tempPath = null;
        ...
        return [
            ...
            'extension' => $extension,
            'hash_name' => $hashName,
            'file_size' => (int) (is_file($originalPath) ? filesize($originalPath) : 0),
        ];
    }
    ...
    /**
     * @param  iterable<int, string|null>  $compoundNames
     */
    public function deleteMany(iterable $compoundNames): void
    {
