        ...
        @if ($featuredHotels->isNotEmpty())
            @if ($featuredHotels->count() > 4)
                <div class="relative py-4">
                    ...
                    <button id="offers-prev" type="button" class="absolute left-2 top-[calc(50%-1rem)] z-10 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-[#EAFFE1] shadow-[2px_8px_28px_0_rgba(0,0,0,0.16)] transition hover:bg-[#ddf7d4] sm:left-4">
                        <svg class="h-4 w-4" fill="none" stroke="#4CAF20" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M15 19l-7-7 7-7" />
                        </svg>
                    </button>
                    <button id="offers-next" type="button" class="absolute right-2 top-[calc(50%-1rem)] z-10 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-[#EAFFE1] shadow-[2px_8px_28px_0_rgba(0,0,0,0.16)] transition hover:bg-[#ddf7d4] sm:right-4">
                        <svg class="h-4 w-4" fill="none" stroke="#4CAF20" viewBox="0 0 24 24">
            ...
            @else
                <div class="grid grid-cols-1 gap-8 py-2 lg:grid-cols-2 xl:grid-cols-3 justify-items-center">
                    @foreach ($featuredHotels as $hotel)
                        <x-hotel-card :hotel="$hotel" />
                    @endforeach
                </div>
        ...
        <section class="relative w-full" aria-label="{{ $offer->name }}">
            ...
            @if ($offer->link)
                <a
                    ...
                    class="absolute bottom-6 right-6 inline-flex items-center gap-2 rounded-lg bg-green-300 px-4 py-2 text-base font-bold font-inter text-white transition hover:bg-green-400 sm:bottom-8 sm:right-8"
                >
                    Ver más
                </a>
            @endif
