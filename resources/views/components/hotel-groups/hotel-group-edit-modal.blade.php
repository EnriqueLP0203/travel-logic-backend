@props([
    'name' => 'hotel-group-edit',
])

@php
    $isEditError = $errors->any() && old('_method') === 'PUT';
    $editId = old('hotel_group_id');
@endphp
...
<div
    ...
    <div class="relative z-10 flex w-full max-w-3xl flex-col rounded-lg bg-white shadow-xl">
        ...
        <form
            data-edit-form
            method="POST"
            action="{{ $editId ? route('admin.hotel-groups.update', $editId) : '#' }}"
            enctype="multipart/form-data"
            class="flex flex-col">
            @csrf
            @method('PUT')
            <input type="hidden" name="hotel_group_id" data-edit-id value="{{ $editId }}">
